        ...
        {[...Array(daysInMonth)].map((_, i) => {
          ...
          return (
            ...
            >
              ...
              {hasTrades && (
                <span className={`mt-auto font-semibold ${summary.pnl >= 0 ? 'text-emerald-700' : 'text-red-600'}`}>
                  {summary.pnl >= 0 ? '+' : ''}{formatCurrency(summary.pnl)}
                </span>
              )}
              {isNoTrade && <span className="mt-auto text-gray-400 italic text-[10px]">No Trade</span>}
            </div>
          )
        })}
      </div>
        ...
        {selectedDay && (
          <div>
            {selectedDay.no_trade ? (
              <p className="text-gray-500 text-sm">No trade recorded for this day.</p>
            ) : selectedDay.trades.length === 0 ? (
              <p className="text-gray-500 text-sm">No trades found.</p>
            ) : (
              <div className="space-y-3">
                <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                  <span className="text-sm text-gray-500">{selectedDay.trades.length} trade{selectedDay.trades.length !== 1 ? 's' : ''}</span>
                  <span className={`font-semibold ${selectedDay.pnl >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                    Total: {formatCurrency(selectedDay.pnl)}
                  </span>
                </div>
                {selectedDay.trades.map(trade => (
                  <div key={trade.id} className="bg-gray-50 rounded-lg p-3 space-y-1">
                    <div className="flex justify-between">
                      <span className="font-medium text-gray-900">{trade.asset}</span>
                      <span className={`font-semibold ${trade.pnl >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                    ...
                    {trade.risk_reward != null && <div className="text-xs text-gray-500">R/R: {trade.risk_reward.toFixed(2)}</div>}
                    {trade.notes && <div className="text-xs text-gray-600 mt-1 italic">&quot;{trade.notes}&quot;</div>}
                  </div>
                ))}
              </div>
            )}
